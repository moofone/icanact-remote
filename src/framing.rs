...
pub const ASK_RESPONSE_HEADER_LEN: usize = 12; // type(1) + correlation_id(4) + pad(7)
pub const GOSSIP_HEADER_LEN: usize = 12; // type(1) + pad(11)
pub const ACTOR_HEADER_LEN: usize = 28; // type(1) + correlation_id(4) + schema_hash(8) + reserved(3) + actor_id(8) + type_hash(4)
pub const STREAM_HEADER_PREFIX_LEN: usize = 13; // type(1) + correlation_id(4) + schema_hash(8)

pub const SCHEMA_HASH_LEN: usize = 8;

// DirectAsk fast path constants
pub const DIRECT_ASK_HEADER_LEN: usize = 12; // type(1) + correlation_id(4) + payload_len(4) + pad(3)
pub const DIRECT_RESPONSE_HEADER_LEN: usize = 12; // type(1) + correlation_id(4) + payload_len(4) + pad(3)
...
pub fn write_actor_frame_header(
    ...
    header[20..28].copy_from_slice(&actor_id.to_be_bytes());
    header[28..32].copy_from_slice(&type_hash.to_be_bytes());
    header
}
...
mod tests {
    ...
    fn codec_trap_length_prefix_stripped_requires_more_padding() {
        let ask_header_without_pad = 1 + 4; // type + correlation_id
        let ask_pad_needed = (ALIGNMENT - (ask_header_without_pad % ALIGNMENT)) % ALIGNMENT;
        assert_eq!(ask_pad_needed, 11);
        ...
        // The actor header includes an 8-byte schema hash plus 3 reserved bytes.
        let actor_header_without_pad = 1 + 4 + SCHEMA_HASH_LEN + 8 + 4;
        let actor_header_with_length_prefix = LENGTH_PREFIX_LEN + actor_header_without_pad;
        let actor_pad_needed_for_32 = 32 - actor_header_with_length_prefix;
        assert_eq!(actor_pad_needed_for_32, 3);
    }

    #[test]
    fn write_ask_response_header_sets_length_and_pad() {
        let payload_len = 3;
        let header = write_ask_response_header(MessageType::Ask, 0x1234_5678, payload_len);
    ...
    fn actor_header_encodes_schema_hash() {
        ...
        let reserved = &header[9..20];
        assert_eq!(&reserved[..SCHEMA_HASH_LEN], &schema_hash.to_be_bytes());
        assert_eq!(&reserved[SCHEMA_HASH_LEN..], &[0u8; 3]);
