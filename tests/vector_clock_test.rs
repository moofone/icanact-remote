use icanact_remote::{ClockOrdering, SecretKey, VectorClock};
...
async fn test_vector_clock_in_actor_location() {
    use icanact_remote::RemoteActorLocation;
    use std::net::SocketAddr;

    let addr: SocketAddr = "127.0.0.1:8080".parse().unwrap();
    let peer_id = icanact_remote::KeyPair::new_for_testing("test_peer").peer_id();
...
fn test_vector_clock_conflict_resolution() {
    ...
    let peer1 = icanact_remote::KeyPair::new_for_testing("peer1").peer_id();
    let peer2 = icanact_remote::KeyPair::new_for_testing("peer2").peer_id();
