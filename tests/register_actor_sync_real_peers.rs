use icanact_remote::{GossipConfig, GossipRegistryHandle, KeyPair, RemoteActorLocation};
...
fn test_peer_id(name: &str) -> icanact_remote::PeerId {
...
async fn ensure_connected(
    ...
    peer_id_a: icanact_remote::PeerId,
    peer_id_b: icanact_remote::PeerId,
) {
...
fn test_register_actor_sync_real_single_peer() {
    run_register_actor_test("single-peer", || async {
        ...
        let handle1 = GossipRegistryHandle::new_with_transport_stack(
            node1_addr,
            test_keypair("node1").to_secret_key(),
            Some(config.clone()),
        ...
        let handle2 = GossipRegistryHandle::new_with_transport_stack(
            node2_addr,
            test_keypair("node2").to_secret_key(),
            Some(config.clone()),
            icanact_remote::BuilderTlsBootstrap,
        )
        .await
...
fn test_register_actor_sync_real_no_peers_vs_with_peers() {
    run_register_actor_test("no-peers-vs-with-peers", || async {
        ...
        let solo_handle = GossipRegistryHandle::new_with_transport_stack(
            solo_addr,
            test_keypair("solo").to_secret_key(),
            Some(config.clone()),
            icanact_remote::BuilderTlsBootstrap,
        )
        .await
    ...
    run_register_actor_test("multiple-peers", || async {
        ...
        for i in 0..4 {
            ...
            let handle = GossipRegistryHandle::new_with_transport_stack(
                addr,
                test_keypair(&node_name).to_secret_key(),
                Some(config.clone()),
                icanact_remote::BuilderTlsBootstrap,
            )
            .await
...
fn test_register_actor_sync_real_peer_timeout() {
    run_register_actor_test("peer-timeout", || async {
        ...
        let handle1 = GossipRegistryHandle::new_with_transport_stack(
            ...
            test_keypair("node1_timeout").to_secret_key(),
            Some(config.clone()),
            icanact_remote::BuilderTlsBootstrap,
        )
        .await
        ...
        let handle2 = GossipRegistryHandle::new_with_transport_stack(
            "127.0.0.1:0".parse().unwrap(),
            test_keypair("node2_timeout").to_secret_key(),
            Some(config.clone()),
            icanact_remote::BuilderTlsBootstrap,
        )
        .await
