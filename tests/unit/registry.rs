...
use icanact_remote::{ActorLocation, GossipConfig, GossipError, KeyPair, RegistrationPriority};
...
async fn test_lookup_actor_ttl() {
    ...
    location.wall_clock_time = icanact_remote::current_timestamp() - 1000; // 1000 seconds ago
...
async fn test_cleanup_stale_actors() {
    ...
    let mut location = create_test_actor_location(actor_addr);
    location.wall_clock_time = icanact_remote::current_timestamp() - 1000; // Very old
