...
/// The same recovery, but after the dead-peer reaper has had its turn.
///
/// The deployed publisher had been failing for three days, so
/// `cleanup_dead_peers` -- which fires once a peer has been at
/// `max_peer_failures` for `dead_peer_timeout` (900s in production) -- had long
/// since run against this peer. The test above never reaches that state: it
/// repairs within a second, well inside any reaper window, so it exercises only
/// the fast path.
///
/// This variant holds the publisher in the failed state past the reaper's
/// window while the subscriber stays up and interested throughout, then asks
/// for the same repair. If reaping a long-dead peer strips state that gossip
/// target selection needs in order to consider the address dialable, the
/// relationship latches: pubsub still routes to the peer, and the only path
/// that could redial it has quietly stopped considering it.
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
async fn publisher_recovers_route_after_dead_peer_reaper_has_run() -> icanact_remote::Result<()> {
    // Long enough that the reaper genuinely fires mid-test, short enough to
    // keep the test fast. Every property under test is a function of "the
    // reaper ran while the peer was still known", not of the exact duration.
    const DEAD_PEER_TIMEOUT: Duration = Duration::from_secs(2);

    let mut config = fast_config();
    config.dead_peer_timeout = DEAD_PEER_TIMEOUT;

    let publisher_keys = KeyPair::new_for_testing("pubsub_reap_publisher");
    let subscriber_keys = KeyPair::new_for_testing("pubsub_reap_subscriber");
    let subscriber_id = subscriber_keys.peer_id();
    ...
    .await?;
    let subscriber = start_node("127.0.0.1:0".parse().unwrap(), subscriber_keys, config).await?;
    let subscriber_addr = subscriber.registry.bind_addr;
    ...
    let topic = topic_key("icanact/pubsub-reconnect/telemetry-after-reap");
    let type_hash: u64 = 0x00D3_2202;
    let received = Arc::new(AtomicUsize::new(0));
    let _sub = {
        let received = Arc::clone(&received);
        pubsub_sub.subscribe_bytes(topic, type_hash, move |_bytes| {
    ...
    // Break the publisher's socket and hold it broken past the reaper window.
    // The subscriber is untouched throughout.
    let _ = publisher
        .registry
        .handle_peer_connection_failure(subscriber_addr, None)
        .await;
    sleep(DEAD_PEER_TIMEOUT + Duration::from_secs(2)).await;
    ...
    let repaired = wait_until(REPAIR_BUDGET, async || {
        pubsub_pub
            .publish_bytes(
                topic,
                type_hash,
                bytes::Bytes::from_static(b"after-reap"),
                PubSubScope::ClusterWide,
                PubSubDeliveryPolicy::default(),
            )
            .map(|stats| stats.remote_enqueued >= 1)
    ...
    assert!(
        repaired,
        "after the dead-peer reaper ran, the publisher never re-established a \
         route to a subscriber that stayed up and interested throughout; last \
         publish was {final_stats:?}"
    );
    ...
    assert!(
        ...
        .await,
        "route reported repaired but no post-reap message ever arrived"
    );
