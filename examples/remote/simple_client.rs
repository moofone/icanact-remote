...
async fn main() -> Result<()> {
    ...
    let server_pub_path = if args.len() > 1 {
        ...
        "/tmp/icanact_tls/simple_server.pub".to_string()
    };
    ...
    let client_key_path = "/tmp/icanact_tls/simple_client.key";
    ...
    let registry = GossipRegistryHandle::new_with_transport_stack(bind_addr, client_secret, Some(config),
