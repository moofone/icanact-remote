...
async fn main() -> Result<()> {
    ...
    println!("🔐 TLS-Enabled Secure Icanact Remote Server");
    println!("==========================================\n");
    
    // 1. Load or generate server TLS keypair
    println!("1. Setting up server TLS identity...");
    let server_key_path = "/tmp/icanact_tls/tell_secure_server.key";
    ...
    let registry = GossipRegistryHandle::new_with_transport_stack(server_addr, server_secret, Some(config),
