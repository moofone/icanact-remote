...
async fn main() -> Result<()> {
    ...
    let server_pub_path = if args.len() > 1 {
        ...
        "/tmp/icanact_tls/tell_secure_server.pub".to_string()
    };
    
    println!("🔐 TLS-Enabled Secure Icanact Remote Client");
    println!("==========================================\n");
    ...
    let client_key_path = "/tmp/icanact_tls/tell_secure_client.key";
    ...
    let registry = GossipRegistryHandle::new_with_transport_stack(client_addr, client_secret, Some(config),
