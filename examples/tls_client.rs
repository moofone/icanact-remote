...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            std::env::var("RUST_LOG")
                .unwrap_or_else(|_| "icanact_remote=info,tls_client=info".to_string()),
        )
        .init();
    ...
    if args.len() < 2 {
        ...
        eprintln!();
        eprintln!("Examples:");
        eprintln!("  {} 9001 /tmp/icanact_tls_server.pub", args[0]);
        eprintln!(
            "  {} 9001 /tmp/icanact_tls_server.pub /tmp/client1.key",
            args[0]
        );
        eprintln!("  {} 9001 /tmp/icanact_tls_server.pub --wrong-key", args[0]);
        return Ok(());
    }
    ...
    let server_pub_path = args
        .get(2)
        .map(|s| s.as_str())
        .unwrap_or("/tmp/icanact_tls_server.pub");
    ...
    let (client_secret_key, client_key_source) = if use_wrong_key {
        ...
        let key_path = client_key_path.unwrap_or("/tmp/icanact_tls_client.key");
    ...
    let registry =
        GossipRegistryHandle::new_with_transport_stack(bind_addr, client_secret_key, Some(config), icanact_remote::BuilderTlsBootstrap).await?;
